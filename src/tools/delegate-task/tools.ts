...
export function resolveCategoryConfig(
  ...
  const { userCategories, inheritedModel, systemDefaultModel, sisyphusJuniorModel } = options
  ...
  // Model priority: category override > agent override > inherited from parent > system default
  const model = resolveModel({
    userModel: userConfig?.model ?? sisyphusJuniorModel,
    inheritedModel,
...
export function createDelegateTask(options: DelegateTaskToolOptions): ToolDefinition {
  ...
  return tool({
    ...
    args: {
      ...
      subagent_type: tool.schema.string().optional().describe("Agent name directly (e.g., 'K9 - advisor', 'X1 - explorer'). Mutually exclusive with category."),
