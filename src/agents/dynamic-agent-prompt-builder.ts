...
export function buildToolSelectionTable(
  ...
  rows.push("")
  rows.push("**Default flow**: X1/R2 (background) + tools -> K9 (if required)")
...
export function buildExploreSection(agents: AvailableAgent[]): string {
  ...
  return `### X1 Explorer = Contextual Grep
...
export function buildLibrarianSection(agents: AvailableAgent[]): string {
  ...
  return `### R2 Researcher = Reference Grep
...
\`\`\`typescript
delegate_task(category="...", load_skills=[], run_in_background=true, prompt="...")  // Empty load_skills without justification
\`\`\``
}
...
export function buildOracleSection(agents: AvailableAgent[]): string {
  ...
  return `<K9_Usage>
## K9 - Read-Only High-IQ Consultant

K9 is a read-only, expensive, high-quality reasoning model for debugging and architecture. Consultation only.

### WHEN to Consult (K9 FIRST, then implement):
...
### Usage Pattern:
Briefly announce "Consulting K9 for [reason]" before invocation.

**Exception**: This is the ONLY case where you announce before acting. For all other work, start immediately without status updates.

### K9 Background Task Policy:

**You MUST collect K9 results before your final answer. No exceptions.**

- K9 may take several minutes. This is normal and expected.
- When K9 is running and you finish your own exploration/analysis, your next action is \`background_output(task_id="...")\` on K9 - NOT delivering a final answer.
- K9 catches blind spots you cannot see - its value is HIGHEST when you think you don't need it.
- **NEVER** cancel K9. **NEVER** use \`background_cancel(all=true)\` when K9 is running. Cancel disposable tasks (X1, R2) individually by taskId instead.
</K9_Usage>`
}
...
export function buildHardBlocksSection(): string {
  const blocks = [
    ...
    "- `background_cancel(all=true)` when K9 is running - **Never.** Cancel tasks individually by taskId.",
    "- Delivering final answer before collecting K9 result - **Never.** Always `background_output` K9 first.",
  ]
...
export function buildAntiPatternsSection(): string {
  const patterns = [
    ...
    "- **K9**: Skipping K9 results when K9 was launched - ALWAYS collect via `background_output`",
  ]
...
export function buildUltraworkSection(
  ...
  if (agents.length > 0) {
    const ultraworkAgentPriority = ["X1 - explorer", "R2 - researcher", "Musashi - plan", "K9 - advisor"]
