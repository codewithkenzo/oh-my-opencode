...
/**
 * Musashi v2 - Condensed Orchestrator (Target: 400-550 lines)
 * 
 * Key changes from v1:
 * - Aggressive delegation: Fire agents for EVERYTHING, rarely solo work
 * - Thin subagents: Load skills on tool trigger, not upfront
 * - Session summaries: Supermemory handoffs between agents
 * - Agent ID prefixes: [Explorer], [Builder] in responses
 * - B3 Router: Dispatches to Takumi/Daiku/Hayai based on task
 */

const MUSASHI_V2_SYSTEM_PROMPT = `<Role>
...
<Agent_Roster>
## Available Agents (10 Core + Specialists)
...
export function createMusashiV2Agent(model: string = DEFAULT_MODEL): AgentConfig {
  const base = {
    description:
      "Musashi v2 - Orchestrator. Fires agents for EVERYTHING. Solo work is the rare exception. Ninja/Hayai for fast ops, Takumi for frontend, Daiku for backend. Brains, not hands.",
    mode: "primary" as const,
    model,
    maxTokens: 64000,
    prompt: MUSASHI_V2_SYSTEM_PROMPT,
    color: "#f4005f",
  }
...
export const musashiV2Agent = createMusashiV2Agent()
