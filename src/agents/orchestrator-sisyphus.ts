...
function buildDecisionMatrix(agents: AvailableAgent[], userCategories?: Record<string, CategoryConfig>): string {
  ...
  if (agentNames.includes("K9 - advisor")) rows.push("| Code review / architecture | `agent=\"K9 - advisor\"` |")
...
\`\`\`typescript
delegate_task(agent="K9 - advisor", prompt="...")     // Expert consultation
delegate_task(agent="X1 - explorer", prompt="...")    // Codebase search
delegate_task(agent="R2 - researcher", prompt="...")  // External research
\`\`\`
...
3. Invoke that agent using \`delegate_task()\` or \`call_omo_agent()\` - NEVER use OpenCode's native Task tool
