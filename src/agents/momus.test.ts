...
describe("MOMUS_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing .musashi/plans/ and ending in .md", () => {
    // #given
    const prompt = MOMUS_SYSTEM_PROMPT

    // #when / #then
    expect(prompt).toContain(".musashi/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .musashi/plans/plan.md"
