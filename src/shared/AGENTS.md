...
36 cross-cutting utilities: path resolution, token truncation, config parsing, model resolution, agent display names, legacy migration.
...
├── migration.ts             # Legacy config + agent name migration
├── opencode-version.ts      # Version comparison
├── external-plugin-detector.ts # OAuth spoofing detection
├── model-requirements.ts    # Agent/Category model requirements
...
├── agent-display-names.ts   # v4 agent display name mapping
├── agent-tool-restrictions.ts # v4 tool restriction helpers
...
├── case-insensitive.ts      # Case-insensitive utils
...
| Migrate legacy names | `AGENT_NAME_MAP`, `BUILTIN_AGENT_NAMES` |
