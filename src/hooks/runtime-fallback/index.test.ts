  ...
  describe("fallback models configuration", () => {
    ...
    test("should use agent-level fallback_models", async () => {
      ...
      const hook = createRuntimeFallbackHook(input, {
        config: createMockConfig({ notify_on_fallback: false }),
        pluginConfig: createMockPluginConfigWithAgentFallback("K9 - advisor", ["openai/gpt-5.2", "google/gemini-3-pro"]),
      })
      ...
      await hook.event({
        event: {
          type: "session.created",
          properties: { info: { id: sessionID, model: "anthropic/claude-opus-4-5", agent: "K9 - advisor" } },
        },
      })
      ...
      await hook.event({
        event: {
          type: "session.error",
          properties: { sessionID, error: { statusCode: 503 }, agent: "K9 - advisor" },
        },
      })
    ...
    test("should detect agent from sessionID pattern", async () => {
      const hook = createRuntimeFallbackHook(createMockPluginInput(), {
        config: createMockConfig({ notify_on_fallback: false }),
        pluginConfig: createMockPluginConfigWithAgentFallback("Musashi", ["openai/gpt-5.2"]),
      })
      const sessionID = "Musashi-session-123"
      ...
      //#then - should detect Musashi from sessionID and use its fallback
      const fallbackLog = logCalls.find((c) => c.msg.includes("Preparing fallback"))
      expect(fallbackLog).toBeDefined()
