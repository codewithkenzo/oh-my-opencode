...
function readTasksConfig(config: Partial<OhMyOpenCodeConfig>): TasksConfig | undefined {
  const musashiConfig = config.musashi_agent
  if (!musashiConfig || typeof musashiConfig !== "object") {
    return undefined
  }

  const tasksValue = (musashiConfig as Record<string, unknown>).tasks
  if (!tasksValue || typeof tasksValue !== "object") {
    return undefined
  }

  const taskRecord = tasksValue as Record<string, unknown>
  return {
    storage_path: typeof taskRecord.storage_path === "string" ? taskRecord.storage_path : undefined,
    task_list_id: typeof taskRecord.task_list_id === "string" ? taskRecord.task_list_id : undefined,
  }
}

export function getTaskDir(config: Partial<OhMyOpenCodeConfig> = {}): string {
  const tasksConfig = readTasksConfig(config)
...
export function resolveTaskListId(config: Partial<OhMyOpenCodeConfig> = {}): string {
  ...
  const configId = readTasksConfig(config)?.task_list_id?.trim()
