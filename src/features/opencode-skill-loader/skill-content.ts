...
export function injectGitMasterConfig(template: string, config?: GitMasterConfig): string {
	...
	sections.push(``)
	sections.push(`Add Musashi attribution to EVERY commit:`)
	sections.push(``)

	if (commitFooter) {
		sections.push(`1. **Footer in commit body:**`)
		sections.push("```")
		sections.push(`Ultraworked with [Musashi](https://github.com/code-yeongyu/oh-my-opencode)`)
		sections.push("```")
		sections.push(``)
	}
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push(`Co-authored-by: Musashi <clio-agent@sisyphuslabs.ai>`)
		sections.push("```")
		sections.push(``)
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Ultraworked with [Musashi](https://github.com/code-yeongyu/oh-my-opencode)" -m "Co-authored-by: Musashi <clio-agent@sisyphuslabs.ai>"`)
		sections.push("```")
	} else if (commitFooter) {
		sections.push(`**Example:**`)
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Ultraworked with [Musashi](https://github.com/code-yeongyu/oh-my-opencode)"`)
		sections.push("```")
	} else if (includeCoAuthoredBy) {
		sections.push(`**Example:**`)
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Co-authored-by: Musashi <clio-agent@sisyphuslabs.ai>"`)
		sections.push("```")
	}
